...
const { PLATFORM_TAG, MKT_OPERATE_LOG } = require('../../../../config/setting');
...
async function list(query, userInfo) {
  ...
  let knexSql = knex(PLATFORM_TAG).select('*')
  if (query.status) {
...
async function add(body, userInfo) {
  ...
  let checkData = await knex(PLATFORM_TAG).select('*').where('name', body.name)
  ...
  await knexTransaction(async (trx) => {
    if (!checkData.length) {
      let max_id = (await trx.max('id as max_id').from(PLATFORM_TAG).where('id', '<', 99999))[0]
      let insertObj = {
      ...
      let id = (await trx(PLATFORM_TAG).insert(insertObj))[0]
      retu.data = id
      ...
      await trx(PLATFORM_TAG).update(updateObj).where('id', checkData[0].id)
...
async function edit(body, userInfo) {
  ...
  if (body.name) {
    let checkData = await knex(PLATFORM_TAG).select('*').where('name', body.name).whereIn('status', [1, 2])
    if (checkData.length && checkData[0].id != body.id) throw new Error(`品类[${body.name}]已经存在，请勿修改为已存在的品类名`)
  }
  let oldData = await knex(PLATFORM_TAG).select('*').where('id', body.id)
  ...
  await knexTransaction(async (trx) => {
    ...
    if (name) updateObj.name = name
    if (remark) updateObj.remark = remark
    await trx(PLATFORM_TAG).update(updateObj).where('id', body.id)
...
async function updateStatus(body, userInfo) {
  ...
  let checkData = await knex(PLATFORM_TAG).select("id", 'status', 'update_user_id').whereIn("id", body.ids)
  if (!checkData.length) return Promise.reject("未查询到品类信息");
  await knexTransaction(async (trx) => {
    await trx(PLATFORM_TAG).update({ status: status, update_user_id: userInfo.id }).whereIn('id', checkData.map(item => item.id))
...
async function del(body, userInfo) {
  ...
  let checkData = await knex(PLATFORM_TAG).select("id", 'status').whereIn("id", body.ids).where("status", "!=", 3);
  if (!checkData.length) throw new Error("未查询到品类信息");
  await knexTransaction(async (trx) => {
    await trx(PLATFORM_TAG)
      .update({ status: 3 })
      .whereIn(
        "id",
...
async function def(query, userInfo) {
  ...
  let knexSql = knex(PLATFORM_TAG).select('*').where('id', query.id)
  let back = await knexSql
