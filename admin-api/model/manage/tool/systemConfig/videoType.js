...
const { VIDEO_TYPE_TAG, MKT_OPERATE_LOG, BUSINESS_TYPE_TABLE } = require('../../../../config/setting');
...
async function list(query = {}, userInfo) {
  query.interface_id = 2815
  await onlyControlInterface(query, userInfo);
  let retu = {
    code: 0,
    count: 0,
    data: [],
  ...
  let knexSql = knex(`${VIDEO_TYPE_TAG} as tag`).select('*').select(selectName('tag', "business_type", BUSINESS_TYPE_TABLE, "name", "business_type_name"))
  if (query.status) {
...
async function add(body, userInfo) {
  if (!body || !body.name || !body.business_type) throw new Error('请传入必填参数')
  ...
  let checkData = await knex(VIDEO_TYPE_TAG).select('*').where('name', body.name).andWhere('business_type', body.business_type)
  if (checkData.length && checkData[0].status == 1) throw new Error(`品类[${body.name}]已经存在，请勿重复添加`)
  let { name, remark, business_type } = body
  await knexTransaction(async (trx) => {
    if (!checkData.length) {
      let max_id = (await trx.max('id as max_id').from(VIDEO_TYPE_TAG).where('id', '<', 99999))[0]
      let insertObj = {
      ...
      let id = (await trx(VIDEO_TYPE_TAG).insert(insertObj))[0]
      retu.data = id
      ...
      await trx(VIDEO_TYPE_TAG).update(updateObj).where('id', checkData[0].id)
      await insertLog(trx, getLogData(body.id, 3014, updateObj, userInfo, checkData[0]))
...
async function edit(body, userInfo) {
  ...
  if (body.name) {
    let checkData = await knex(VIDEO_TYPE_TAG).select('*').where('name', body.name).whereIn('status', [1, 2])
    if (checkData.length && checkData[0].id != body.id) throw new Error(`品类[${body.name}]已经存在，请勿修改为已存在的品类名`)
  }
  let oldData = await knex(VIDEO_TYPE_TAG).select('*').where('id', body.id)
  ...
  await knexTransaction(async (trx) => {
    ...
    if (name) updateObj.name = name
    if (remark) updateObj.remark = remark
    await trx(VIDEO_TYPE_TAG).update(updateObj).where('id', body.id)
    await insertLog(trx, getLogData(body.id, 3015, updateObj, userInfo, oldData[0]))
  })
  return retu
}
...
async function updateStatus(body, userInfo) {
  ...
  let checkData = await knex(VIDEO_TYPE_TAG).select("id", 'status', 'update_user_id').whereIn("id", body.ids)
  if (!checkData.length) return Promise.reject("未查询到品类信息");
  await knexTransaction(async (trx) => {
    await trx(VIDEO_TYPE_TAG).update({ status: status, update_user_id: userInfo.id }).whereIn('id', checkData.map(item => item.id))
    let log_data = checkData.map(item => {
      return getLogData(item.id, 3015, { status: status, update_user_id: userInfo.id }, userInfo, { status: item.status, update_user_id: item.update_user_id })
    })
...
async function del(body, userInfo) {
  ...
  let checkData = await knex(VIDEO_TYPE_TAG).select("id", 'status').whereIn("id", body.ids).where("status", "!=", 3);
  if (!checkData.length) throw new Error("未查询到品类信息");
  await knexTransaction(async (trx) => {
    await trx(VIDEO_TYPE_TAG)
      .update({ status: 3 })
      .whereIn(
        "id",
    ...
    let log_data = checkData.map(item => {
      return getLogData(item.id, 3016, { status: 3 }, userInfo, { status: item.status })
    })
    await insertLog(trx, log_data)
  })
  return {
...
async function def(query, userInfo) {
  ...
  let knexSql = knex(VIDEO_TYPE_TAG).select('*').where('id', query.id)
  let back = await knexSql
