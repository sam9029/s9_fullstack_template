...
// do request example
async function doRequest(phone, code) {
    let body = {
        SmsAccount: "7bc9f92e",
        Sign: "小果繁星",
        TemplateID: "ST_7bdbacb2",
        PhoneNumbers: phone,
    ...
    const responseText = await request({
        url: `https://iam.volcengineapi.com/?${qs.stringify(signParams.query)}`,
    ...
    if (responseText?.ResponseMetadata?.Error) {
        console.log(responseText?.ResponseMetadata);
        return Promise.reject(responseText?.ResponseMetadata?.Error?.Message || '短信发送失败')
    }
}
// doRequest('17396983034', '123456')
