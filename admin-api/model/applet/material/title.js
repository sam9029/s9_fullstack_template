...
async function list(query = {}, userInfo = {}) {
  ...
  let sql = knex
    .select("title.*")
    .from(`${TITLE_TABLE} as title`)
    .where("title.oem_id", userInfo.oem_id)
    .andWhere((builder) => {
  ...
  let data = await sql.select(
    selectName("title", "task_id", TASK_TABLE, "name", "task_name")
  );
...
async function add(query = {}, userInfo = {}) {
  let res = await knexTransaction(async (trx) => {
    ...
    let result = await trx.insert(insert).from(`${TITLE_TABLE}`);
    let log_data = getLogData(result[0], 5501, insert, userInfo, {});
...
async function update(query = {}, userInfo = {}) {
  let res = await knexTransaction(async (trx) => {
    ...
    await trx.update(update).from(`${TITLE_TABLE}`).whereIn("id", query.ids);
  ...
  async searchFilter(query, builder) {
    if (query.keyword) {
      builder.where((builder_copy) => {
        builder_copy
          .where("title.id", "like", `%${query.keyword}%`)
          .orWhere("title.content", "like", `%${query.keyword}%`);
      });
    }
    if (query.id) {
      builder.where("title.id", query.id);
    }
    if (query.status) {
      builder.where("title.status", query.status);
    }
    if (query.task_id) {
      builder.where("title.task_id", query.task_id);
    }
  },
};
