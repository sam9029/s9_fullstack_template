...
const { APPROVAL_PROCESS_V2, PROMOTED } = require("../config/setting");
...
class ApprovalLockUtil {
  data_table = "";
  back_verify_pass_id = false //返回审核通过的ID
  // trx = knex;
  log_type = 0;
  approval_type = null
  userinfo = null;
  ...
  constructor(opts) {
    ...
    if (opts) {
      ...
      this.back_verify_pass_id = opts.back_verify_pass_id
  ...
  async init(userinfo, trx = knex) {
    ...
    const approval_raw = (await trx(APPROVAL_PROCESS_V2).select("id", "config")
  ...
  async batch_approval(raws = [], verify_status, verify_suggest, userinfo = {}, default_release_lock = true, trx) {
    const success = [], fails = [], verify_pass = [];
    ...
    for (let index = 0; index < chunk_array.length; index++) {
      ...
      let all_array = group_data.map(async element => {
        return await this.approval(element, verify_status, verify_suggest, userinfo, default_release_lock, trx).then(data_raw => {
          ...
          if (this.back_verify_pass_id && data_raw.update_verify_status == 3) verify_pass.push(data_raw?.id)
        }).catch(err => {
    ...
    return { success, fails, verify_pass }
  }
  /**
  ...
  get_submit_info(data_raw) {
    ...
    const update_obj = {
      verify_status: 2,
      next_account_id: JSON.stringify(people_info.id || []),
  ...
  async approval_verify(data_raw, verify_status, verify_suggest, trx, userinfo) {
    ...
    switch (verify_status) {
      case 3: // 通过
        ...
        let id = approval_process[new_step]?.id || []
        update_obj.next_account_id = Array.isArray(id) ? JSON.stringify(id) : String(id);
      ...
      case 4: // 拒绝
        ...
        update_obj.next_account_id = JSON.stringify([]);
        break;
      case 5: // 召回
    ...
    if (this.back_verify_pass_id) data_raw.update_verify_status = update_obj.verify_status
    return data_raw
  }
  ...
  check_approval_account(data_raw, verify_status) {
    ...
    // 审核
    if (verify_status != 5) {
      let next_account_id = JSON.parse(data_raw.next_account_id)
      if (Array.isArray(next_account_id)) {
        if (!next_account_id.includes(user_id)) throw "非审批人操作！"
      } else if (next_account_id != user_id) throw "非审批人操作！"
    } else {
      // 召回
