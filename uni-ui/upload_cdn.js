...
let minioBucket = {
    ...
    // port: 8000,
    // useSSL: false,
    // accessKey: 'YbIclY8fU4uBO6fV64oC',
    // secretKey: '8xYG8AtVTmplRa4tpBeFDZA77FT6gRp3rBfYp0ki',
    ...
    accessKey: 'noneKey',
    secretKey: 'noneKey',
    openBucketName: 'duolai-img',
    public_path: 'open-img/',
}
...
function readFileList(file_path, filesList) {
    ...
    files.forEach(function (itm, index) {
        ...
        if (stat.isDirectory()) {//递归读取文件
            ...
            obj.filename = itm//名字
            obj.ossfilename = 'cdn/' + project_name + '/static/' + String(file_path).replace(String(path.join(__dirname, './unpackage/dist/build/web/static/')), '') + itm
            filesList.push(obj);
        }
    })
}
// obj.ossfilename = public_path + String(file_path).replace(String(path.join(__dirname, '')), '') + itm
// let file_path = path.join(__dirname, './unpackage/dist/build/h5/static/')
const store = new OSS(bucket);
async function uploadCdn() {
    let file_path = path.join(__dirname, './unpackage/dist/build/web/static/')
...
function loadEnv(mode) {
    ...
    if (mode == 'production') return { VUE_CDN_SITE: 'WANGSU' }
    else return { VUE_CDN_SITE: 'WANGSU' }
}
