<template>
	<view class="xingguang-page">
		<view class="top-area">
			<MyNavbar bgColor="transparent" :leftIcon="`${static_path}circular_back.png`"
				:rightIcon="`${static_path}nav_btn_icon.png`" rightBtn rightBtnText="问题解答" :rightClick="goQS">
				<template #navbarData>
					<text class="u-font-32 color-text-black u-font-bold" style="margin-left: 16rpx">星广联投</text>
				</template>
			</MyNavbar>
			<view class="header-tab u-p-l-28 u-p-r-28 u-m-t-28">
				<view class="tab-item u-font-bold" :class="{ 'tab-item--active': tabIndex === 1 }"
					style="margin-right: 32rpx" @click="changeTab(1)">任务列表</view>
				<view class="tab-item u-font-bold" :class="{ 'tab-item--active': tabIndex === 2 }"
					@click="changeTab(2)">我的任务</view>
			</view>
			<view class="u-flex-row u-m-t-44 u-m-l-28 u-m-r-28 u-row-between u-col-center" v-show="tabIndex === 2">
				<text class="u-text-main u-font-32">任务列表</text>
				<view class="switch-bg">
					<text class="switch-item" :class="{ 'switch-item-active': switchIndex === 1 }"
						@click="changeStatus(1)">进行中</text>
					<text class="switch-item" :class="{ 'switch-item-active': switchIndex === 2 }"
						@click="changeStatus(2)">已结束</text>
				</view>
			</view>
		</view>
		<view class="u-p-l-28 u-p-r-28 u-m-t-28">
			<view v-if="listData.length">
				<view v-for="(item, index) in listData" :key="item.id" class="card-bg u-m-b-16 u-flex-row u-row-between"
					@click="goDetail(item)">
					<view class="u-flex-row">
						<u-image :src="item.advertiser_icon" mode="scaleToFill" shape="square" radius="32rpx"
                ...
                `任务周期：${item.start_date} -${item.end_date}`
                  ...
                  overflow-x: hidden;
                " class="u-flex-row">
								<view class="u-flex-row u-col-center" v-for="(policy, index) in item.policys"
									:key="policy.id">
									<view class="u-p-r-8 u-p-l-8 u-nowrap u-font-24" :style="{
                      color: policy.show_name == '保底' ? '#408CFF' : '#3C3C3C',
                    }">
										{{ policy.show_name }}
									</view>
									<u-line v-show="index != item.policys.length - 1" direction="col" length="16rpx"
										color="#C6C6C6"></u-line>
								</view>
							</view>
							<view style="
                ...
                " class="u-flex-row u-col-center">
								...
								<view class="u-m-l-8 u-m-r-8 u-font-24"
					...
					<u-icon class="right-button" :name="`${static_path}blue_right_arrow.png`" width="32rpx"
						height="32rpx"></u-icon>
				</view>
			</view>
		...
		<u-popup :show="showVideo" round="8" bgColor="#F6F7FB">
			<view class="u-flex-row u-row-between u-col-center u-p-l-28 u-p-r-28 u-p-t-16">
				...
				<u--image @click="closeVideo" :src="`${static_path}close_circle_grey.png`" width="48rpx"
					height="48rpx"></u--image>
			</view>
			<view class="u-p-l-28 u-p-r-28 u-p-b-28 u-p-t-28">
				<!-- #ifndef APP -->
				<video :src="currentVideo" :poster="currentVideoPoster" object-fit="contain" enable-danmu danmu-btn
					controls controlslist="nodownload noremoteplayback" class="u-border-radius" :style="{
	        width: '100%',
	        height: '400rpx',
	      }"></video>
				<!-- #endif -->
...
<script>
	...
	export default {
		...
		computed: {
			...
			videoSource() {
				return `<video src="${this.currentVideo}" poster="${this.currentVideoPoster}" object-fit="cover" enable-danmu  danmu-btn controls controlslist="nodownload noremoteplayback" class="u-border-radius app-video" style="width: 100%; height: 200px;"></video>`;
			},
		},
		methods: {
			closeVideo() {
			...
			goQS() {
				if(this.qsData!=null){
					switch (this.qsData.type) {
						case 1:
							if (
								[
									...
									"pages/income/income",
									"pages/mine/mine",
								].includes(this.qsData.content)
							) {
								uni.switchTab({
									url: this.qsData.content,
								});
							} else {
								uni.navigateTo({
									url: this.qsData.content,
						...
						case 2:
							uni.navigateTo({
								url: `/pages/webView/index?url=${this.qsData.content}`,
							});
							break;
						case 3:
							this.currentVideo = this.qsData.content;
							this.currentVideoPoster = this.qsData.url;
							this.showVideo = true;
							break;
						case 4:
							uni.navigateTo({
								url: `/pages/parse/index?id=${this.qsData.banner_id}&type=DEFAULT`,
							});
							break;
					}
