<template>
  <view class="account-page u-p-x-28">
    <view class="u-bg-f u-border-radius u-p-48 u-m-y-28">
      ...
      >
        <u--image
          :src="model.avatar || `${static_path}avatar_upload_icon.png`"
          width="200rpx"
          height="200rpx"
          shape="circle"
        ></u--image>
      ...
      >
        ...
        <u-form-item label="我的账户ID">
          <view class="u-flex-row u-row-between u-col-center widthAll">
            ...
            <text
              class="color-text-primary u-font-24 u-line-h-44"
              @click="onCopy(model.id)"
              >复制</text
            >
          </view>
        </u-form-item>
        <u-form-item label="联系导师（微信号）">
          <view class="u-flex-row u-row-between u-col-center widthAll">
            ...
            <text
              class="color-text-primary u-font-24 u-line-h-44"
              @click="onCopy(model.contact_teacher)"
              >复制</text
            >
          </view>
        </u-form-item>
    ...
    <u-button type="primary" :loading="loading" @click="showGate = true"
      >保存修改</u-button
    >
    <u-modal
      :show="showGate"
...
<script>
import { upload_file, getUserInfo } from "@/api/public";
import { getLeaderWechat } from '@/api/account.js'
import { compress, throttle, copy } from "@/utils/tools.js";
import { postEditInfo } from "../api/userInfo.js";
  ...
  methods: {
    ...
    async uploadFilePromise(file, name) {
      ...
      return new Promise((resolve, reject) => {
        // 调用上传文件的API
        upload_file({ path_type: "avatar", file_name: file.name }, file.url)
          .then((res) => {
