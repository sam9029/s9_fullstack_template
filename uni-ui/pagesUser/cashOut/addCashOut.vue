<template>
  <view class="add-cash-out">
      ...
      <view class="u-border-radius u-bg-f u-p-32">
        ...
        >
          ...
          >
            <u-input
              placeholder="请输入开户行"
              v-model.trim="model.bank_deposit"
            >
            </u-input>
          </u-form-item>
          <u-form-item
            :label="currentTab == 'ALIPAY' ? '支付宝账号' : '银行卡号'"
            prop="pay_account"
            required
          >
            <u-input
              :placeholder="
                currentTab == 'ALIPAY' ? '请输入支付宝账号' : '请输入银行卡号'
              "
          ...
          <u-form-item>
            <text class="color-text-black u-font-32 u-line-h-48 u-font-bold">{{
              currentTab == "COMPANY" ? "上传营业执照" : "上传身份证照片"
            }}</text>
          </u-form-item>

					<view v-if="currentTab != 'COMPANY'" class="u-flex-row u-col-center">
						<u-form-item :prop="currentTab != 'COMPANY'?model.id_card_face_url:''">
						...
						<u-form-item :prop="currentTab != 'COMPANY'?model.id_card_back_url:''">
					...
					<u-form-item v-else :prop="currentTab == 'COMPANY'?model.id_card_face_url:''">
					...
					<u-form-item>
						<view class="u-flex-row u-col-center">
							...
							<view class="color-text-less-grey u-font-24">
								本人承诺以阅读并同意<text
									class="color-text-primary" @click="privacy">《隐私政策协议》</text>{{currentTab != 'COMPANY'?"和":""}}<text
									v-if="currentTab != 'COMPANY'"
									class="color-text-primary" @click="electronicSigningAgreement">《电签协议》</text>，本人按照协议内容向贵司提供相应服务
							</view>
						</view>
					</u-form-item>
				</u--form>
    ...
    <u-modal
      ...
      :title="`${currentTab != 'COMPANY' ? '用户协议&隐私政策' : '隐私政策'}`"
      confirmText="同意"
      cancelText="不同意"
...
import { upload_file, getBankConfig } from "@/api/public.js";
import { compress, throttle, removeObjectEmpty, sleep } from "@/utils/tools.js";
import { postBankSign, postBankCheck, getBankDef } from "../api/cashOut.js";
...
export default {
  ...
  methods: {
          ...
          this.toastMsg(error.message || "验证失败！", "error");
			...
			beforeSubmit: throttle(function func() {
				this.$refs.formRef
					...
					.catch((error) => {
						this.toastMsg(`error:${JSON.stringify(error)}`, "error");
					});
			}, 500), // 500毫秒的防抖时间
			sureBind() { //确认绑定
        ...
        removeObjectEmpty({
          ...
          people_name:
            this.currentTab == "COMPANY"
              ...
              : this.model.people_name,
          mobile:
            this.currentTab == "COMPANY"
              ? this.model.bank_deposit
              : this.model.mobile,
          id_card:
            this.currentTab == "COMPANY"
	...
	electronicSigningAgreement() {
		uni.navigateTo({
		  url: "/pagesUser/service/agreement/other", // 电签协议页面的URL
		});
	},
