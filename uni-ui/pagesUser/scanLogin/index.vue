<template>
  <view>
    ...
    <view class="centered-flex">
      <u--image
        :src="`${static_path}icon_computer.png`"
        width="320rpx"
        height="320rpx"
        class="u-m-t-28"
      ></u--image>
    </view>
    <view class="u-text-main u-font-32 u-line-h-48" style="text-align: center"
      >PC端登录确认</view
    >
    <view
      class="color-text-less-grey u-font-24 u-line-h-40"
      style="text-align: center"
      >请核对登录设备信息，切勿在可疑设备登录</view
    >
    ...
    <view class="u-flex-row u-m-x-28" style="margin-top: 128rpx">
      ...
      <view class="btn-bg-blue" @click="handleSubmit">确认登录</view>
		...
		<AfterScan v-model="showAfterScan" />
...
import { postScanLogin, postScanLocation } from "@/api/login.js";
...
export default {
  ...
  methods: {
    queryScanLocation() {
      postScanLocation({ time: this.scan_data.time, uuid: this.scan_data.uuid })
        .then((res) => {
          if (res.code == 0) {
        ...
        .catch((err) => {
          this.toastMsg(err || err.message, "error");
        });
    },
