<template>
  ...
  >
    <view class="u-modal">
      <view class="u-modal__icon" v-if="icon">
        <image class="u-modal__icon__image" :src="icon"></image>
      </view>
      <text
        v-if="title"
        class="u-modal__title"
        :class="{ 'u-modal__title_center': icon || buttonFill }"
        :style="{
          paddingTop: `${icon ? 32 : 0}rpx`,
          fontWeight: 'bold',
        }"
        >{{ title }}</text
      >
      <view
        ...
        :class="{ 
					'u-modal__content__center':  icon || buttonFill,
					'u-modal__content__left': !icon && !buttonFill
				}"
      >
        <scroll-view scroll-y="true" class="u-modal__content__scroll">
          <slot>
            <text class="u-modal__content__scroll__text">{{ content }}</text>
          </slot>
        </scroll-view>
      </view>
      <view
      ...
      <template v-else>
        ...
        >
          <view
            ...
            style="border-radius: 16rpx; background-color: #f6f6f6"
            v-if="showCancelButton"
            @tap="cancelHandler"
          >
            <text
          ...
          <view
            class="u-modal__button-group__wrapper u-modal__button-group__wrapper--confirm"
            style="border-radius: 16rpx; background-color: #408cff"
            v-if="showConfirmButton"
            @tap="confirmHandler"
          >
            <u-loading-icon v-if="loading"></u-loading-icon>
