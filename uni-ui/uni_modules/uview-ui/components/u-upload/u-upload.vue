<template>
  <view class="u-upload" :style="[$u.addStyle(customStyle)]">
    <view class="u-upload__wrap">
      <template v-if="previewImage">
        <view
          ...
          :class="{
            'u-upload__wrap__preview__fit': item.status == 'failed',
          }"
    ...
    <u-modal :show="showCameraAuth" :showConfirmButton="false">
      <view class="camera-auth">
        <view class="camera-auth--top">
          <u-icon :name="camera_icon" size="96rpx"></u-icon>
          <text>相机权限未开启</text>
        </view>
        <view
          class="camera-auth--content u-m-y-32 color-text-less-grey u-font-28 u-line-h-44"
        >
        ...
        <view class="camera-auth--bottom u-flex-row u-row-center u-col-center">
          <u-button
            ...
            :customStyle="{ color: '#3C3C3C', 'border-radius': '16rpx' }"
            >我知道了</u-button
          >
          <u-button
            type="primary"
            @click="handleAuthSetting"
