	...
	<view class="create-income-detail">
		<view class="top-card u-p-28">
			<view class="u-bg-f u-flex-row u-col-center u-border-radius u-p-24">
				<u--image :src="detailObj.collection_cover_url" width="132rpx" height="188rpx"
              ...
              detailObj.collection_name || "--"
						...
						<DatePicker title="请选择查询时间段" btnHeight="56rpx" :type="['list']" :showBottomBtns="false"
							@submit="getTopDate" />
					</view>
                ...
                unitMoney(detailObj.account_amount, false, true)
							...
							<view class="u-flex-1 u-flex-col u-row-center" @click="showModal=true">
								<u-icon label="我的收益" labelSize="22rpx" labelColor="#989898"
									:name="`${static_path}icon_gray_qs.png`" width="24rpx" height="24rpx"
		...
		<view class="tabel-navbar u-p-x-28 u-p-t-28 u-p-b-24 u-bg-f u-border-radius">
			<view class="u-flex-row u-col-center">
				<DatePicker title="请选择查询时间段" :type="tableDateType" :baseBtn="false" ref="tableDateRef"
					@submit="getTableDate">
					<template #default="{ label, value }">
						<view class="table-date-picker u-flex-row u-col-center">
							<view class="u-flex-row u-row-center u-col-center heightAll" :class="{
                  'table-date-picker--active': currentTableDateVal == 'default',
                  'table-date-picker--default':
                    currentTableDateVal != 'default',
                }" style="width: 230rpx" @click="switchTableDate('default')">
								<text class="u-font-24 u-line-h-40 u-m-r-8">{{
                  tableDataBtn.label
                }}</text>
								<u-icon name="arrow-down" size="32rpx" :color="
                ...
                }" style="width: 230rpx" @click="switchTableDate('custom')">
			...
			<view class="u-flex-row u-row-between u-col-center u-m-t-16">
				<text class="color-text-less-grey u-font-24 u-line-h-40">查询时间段</text>
		...
		<view class="u-bg-f heightAll">
			<ZbTable :columns="columns" :highlight="true" :data="tableData" :stripe="false" border fit
				:isLoading="tableLoading">
				<template #account_amount="{ row }">
					<text class="color-text-primary" style="text-decoration: underline"
						@click="openIncomePopup(row)">{{ unitMoney(row.account_amount, false, true) }}</text>
				</template>
		...
		<u-popup :show="showCustomTableCols" round="24rpx">
			<view class="u-p-x-28 u-p-t-28" style="display: grid; grid-gap: 32rpx">
				<view class="select-top u-flex-row u-row-between u-col-center">
					<text class="u-font-bold u-font-32 u-line-h-48">自定义查看更多数据</text>
					<u-icon :name="`${static_path}close_circle_grey.png`" size="48rpx"
						@click="closeSeletedCols"></u-icon>
				</view>
				<view class="table-cols-list-btn">
					<view v-for="item in listBtns" :key="item.name"
						class="u-flex-row u-row-center u-col-center u-p-16 u-border-radius" :class="{
              ...
              'table-cols-list-btn--active': selectedCols.includes(item.name),
				...
				<view class="table-cols-bottom-btns">
					<view class="u-flex-row u-row-center u-col-center u-p-y-32 reset-btn widthAll" @click="resetCols"
						style="height: 108rpx">
						<text class="u-font-28 color-text-less-grey">重置</text>
					</view>
					<view class="u-flex-row u-row-center u-col-center u-p-y-32 widthAll" @click="tableColsSubmit"
						style="height: 108rpx">
		...
		<u-popup :show="myIncomePopup" round="16rpx" :closeOnClickOverlay="true" @close="myIncomePopup=false">
			<view class="u-p-28" style="display: grid; grid-gap: 32rpx">
				<view class="select-top u-flex-row u-row-between u-col-center">
					<text class="u-font-bold u-font-32 u-line-h-48">我的收益明细</text>
					<u-icon :name="`${static_path}close_circle_grey.png`" size="48rpx"
						@click="myIncomePopup = false"></u-icon>
				</view>
			</view>
			<view class="u-p-x-28 u-m-t-32 u-p-b-28">
				<view class="u-border-radius u-p-28" style="border: 2rpx solid #eeeeee">
					<view class="u-flex-row u-row-between u-col-center u-m-b-16">
						<view class="u-flex-row u-col-center">
							<u--image :src="currentIncome.app_icon" radius="8rpx" width="32rpx" height="32rpx"
								class="u-m-r-8"></u--image>
							<text class="color-text-black u-font-24 u-line-h-40">{{
                currentIncome.app_name || "--"
              }}</text>
						</view>
						<view class="settle-tag u-p-x-8" :class="{
                ...
                SETTLE_STATUS_MAPPER[currentIncome.settle_status]
					...
					<view class="u-flex-row u-row-between u-col-center u-m-b-16">
						<view class="u-flex-row u-col-center">
							<u--image :src="`${static_path}flow_type_icon.png`" width="32rpx" height="32rpx"
								class="u-m-r-8"></u--image>
							<text class="color-text-less-grey u-font-24 u-line-h-40">流量类型</text>
						</view>
						<text class="color-text-less-grey u-font-24 u-line-h-40">{{
              SOURCE_TYPE_MAPPER[currentIncome.source_type] || "产品引流"
					...
					<view class="u-flex-row u-row-between u-col-center u-m-b-16">
						<view class="u-flex-row u-col-center">
							<u--image :src="`${static_path}pay_platform_icon.png`" width="32rpx" height="32rpx"
								class="u-m-r-8"></u--image>
              ...
              PAY_PLATFORM_MAPPER[currentIncome.pay_platform]
					...
					<view class="income-popup-grid u-p-y-16 widthAll u-border-radius">
						<view class="u-flex-col u-col-center">
							<text
								class="color-text-black u-font-bold u-font-28 u-line-h-44">{{ unitMoney(currentIncome.order_amount, false, true) }}</text>
							<text class="color-text-less-grey u-font-22 u-line-h-40">充值金额</text>
						</view>
						<view class="u-flex-col u-col-center channel_rate">
							<view class="u-flex-row u-col-center">
								<u-icon :name="pay_icon_mapper[currentIncome.pay_platform]" size="28rpx"
									color="#1A1A1A"></u-icon>
								<text
									class="color-text-black u-font-bold u-m-l-8 u-font-28 u-line-h-44">{{ unitRate(currentIncome.platform_rate) }}</text>
							</view>
							<text class="color-text-less-grey u-font-22 u-line-h-40">通道费率</text>
						</view>
						<view class="u-flex-col u-col-center price-border">
							<text
								class="color-text-black u-font-bold u-font-28 u-line-h-44">{{ unitRate(currentIncome.account_ratio) }}</text>
							<text class="color-text-less-grey u-font-22 u-line-h-40">分成比例</text>
						</view>
						<view class="u-flex-col u-col-center">
							<text class="color-text-black u-font-bold u-font-28 u-line-h-44">{{
                  unitMoney(currentIncome.account_amount, false, true)
                }}</text>
							<text class="color-text-less-grey u-font-22 u-line-h-40">我的收益</text>
						</view>
					</view>
				</view>
			</view>
		...
		<u-modal :show="showModal" :showCancelButton="false" :showConfirmButton="true" title="我的收益"
			content="我的收益即剧集充值分成收益，包含待入账预估收益和累计入账收益。" confirmText="我知道了" :buttonFill="false"
			:closeOnClickOverlay="false" @confirm="showModal = false">
		</u-modal>
	</view>
</template>
		...
		methods: {
			...
			tableColsSubmit() {
				if (!this.selectedCols.length) {
					...
					// 只追加不存在的列
					const existingColumnNames = this.columns.map(item => item.name); // 获取当前列表name为数组
					const columnsToAdd = newSelected.filter(el => !existingColumnNames.includes(el.name)); // 过滤不存在的新增数组项
					this.columns = [
