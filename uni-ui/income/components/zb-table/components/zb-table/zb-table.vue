                  ...
                  <view class="item-tr">
                    <view
                      ...
                      :style="{
                        ...
                        flex:
                          maxWidthBigThanCloum ||
                          index === transColumns.length - 1
                            ? 1
                            : 'none',
                    ...
                    >
                      ...
                      <template v-else>
                        {{ item.label || "" }}
                        <view
                          style="opacity: 0; width: 0; display: inline-block"
                          >{{ item.sorterMode }}</view
                        >
            ...
            <view class="zb-table-tbody">
              ...
              >
                <view
                  :style="[
                    {
                      ...
                      flex:
                        maxWidthBigThanCloum || i === transColumns.length - 1
                          ? 1
                          : 'none',
                  ...
                  @click.stop="cellClick(item, index, ite)"
                ...
                >
                  ...
                  <template v-else-if="ite.type === 'custom'">
                    <view class="custom-render">
                      <slot :name="ite.slots.customRender" :row="item"></slot>
                    </view>
                  </template>
                  <template v-else-if="ite.type === 'rate'">
                    {{ unitRate(item[ite.name], ite.unit) }}
                  </template>
                  <template v-else-if="ite.type == 'price'">
                    {{ unitMoney(item[ite.name], ite.unit, ite.thousand) }}
                  </template>
                  ...
                  <template v-else-if="ite.type === 'link'">
                    <text class="color_primary">
                      {{
                        ite.filters
                          ? itemFilter(item, ite)
                  ...
                  <template
                    v-else-if="
                      ite.type == 'needFetch' && ite.filters
                        ? itemFilter(item, ite)
                        : formatterAction(item, ite, index, i)
                    "
                  >
...
export default {
  ...
  computed: {
    ...
    maxWidthBigThanCloum() {
      return this.maxWidth > this.cloumMaxWidth;
    },
    transColumns() {
      if (this.fit) {
      ...
      this.cloumMaxWidth = 0;
      this.columns.forEach((item, index) => {
        this.cloumMaxWidth += item.width || 80;
        if (item.name != this.sorterName) {
          item.sorterMode = undefined;
  ...
  created() {},
  onReady() {
    this.resetCloumWidth();
  },
  methods: {
    renderCustomSlot(item, column) {
      if (column.slots && column.slots.customRender) {
        const slotName = column.slots.customRender;
        return this.$scopedSlots[slotName]
          ? this.$scopedSlots[slotName]({ row: item })
          : null;
      }
      return null;
    },
    resetCloumWidth() {
      let flag = setInterval(() => {
        uni
          .createSelectorQuery()
          .in(this)
          .select(".zb-table-applet")
          .boundingClientRect((data) => {
            this.maxWidth = data.width;
            if (data.width > 0) clearInterval(flag);
          })
          .exec();
      }, 80);
    },
    ...
    cellClick(row, index, { name }) {
      this.$emit("cellClick", row, index, name);
    },
    ...
    sortAction(item, index) {
      ...
      // );
      let model = null;
      if (item.sorterMode === "_asc") model = "_desc";
      if (item.sorterMode === "_desc") model = undefined;
      if (item.sorterMode == undefined) model = "_asc";
      this.$set(item, "sorterMode", model);
      item.sorterMode && (this.sorterName = item.name);

      if (item.sorter === "custom") {
        this.$emit(
          "sort-change",
          item,
          item.sorterMode == undefined ? "" : item.sorterMode.replace("_", ""),
          index
        );
      } else {
        this.sortData(item);
    ...
    unitMoney(money = null, unit = true, thousand = false) {
      if (!money) {
        return unit ? "￥0" : "0";
      }
      ...
      // 如果需要千分位分隔符，手动添加
      if (thousand) {
        formattedMoney = this.addThousandSeparator(formattedMoney);
      } else {
