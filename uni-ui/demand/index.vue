  ...
  <view class="demand-page">
    <view class="top-area">
      <MyNavbar
        bgColor="transparent"
        :backShow="false"
        :rightIcon="`${static_path}close_circle.png`"
        closeShow
      >
        <template #navbarData>
          <view class="u-flex-row u-col-center u-p-l-28">
            <u--image
              class="u-m-r-8"
              :src="currentPlatform.platform_icon"
              width="40rpx"
              height="40rpx"
            ></u--image>
            <text
              class="color-text-black u-relative u-font-bold"
              style="top: -2rpx"
              >{{ currentPlatform.platform_name }}</text
            >
          </view>
        </template>
      </MyNavbar>
    ...
    <view class="u-p-l-28 u-p-r-28">
      <view v-if="listData.length" class="demand-list u-flex-col">
        <listItem
          ...
          :key="index"
          :class="{ 'u-m-b-28': index !== listData.length - 1 }"
          :tab="tabIndex"
          :row="item"
          @actionData="doPromotion"
          @refresh="init"
          @toast="getItemToast"
        ></listItem>
      </view>
