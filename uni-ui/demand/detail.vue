<template>
	<view class="demand-detail-page">
		<view class="top-area u-p-x-28">
			<view class="u-p-y-16 u-flex-row u-col-center " style="background: #f6f7fb;">
				<u-icon :name="`${static_path}circular_back.png`" size="48rpx" @click="goBack"></u-icon>
				<view class="u-flex-row u-row-left u-col-center u-m-l-16">
					<u-icon size="36rpx" :name="currentPlatform.platform_icon" class="u-m-r-8"></u-icon>
		...
		<view class="card-bg u-m-x-28 u-flex-row u-m-t-28">
			<u-image :src="coverImg" width="168rpx" height="232rpx" shape="square" radius="4rpx"></u-image>
            ...
            name
				...
				<view class="u-flex-row u-col-center u-p-l-16 u-p-r-16" style="
            border-radius: 8rpx;
            background: #f6f6f6;
            height: 48rpx;
            width: fit-content;
          ">
					...
					<u-line class="u-p-l-8" color="#C6C6C6" direction="col" length="16rpx"></u-line>
					<text class="u-font-24 color-text-grey u-p-l-8">
						{{ projectStatus }}</text>
				</view>
				<view class="u-flex-row u-p-l-16 u-p-r-16" style="
            border-radius: 8rpx;
            background: #f6f6f6;
            height: 48rpx;
            width: fit-content;
          ">
					<u-icon size="32rpx" :name="`${static_path}icon_plan_mark.png`" :label="projectPlanNum"
						labelSize="24rpx" labelColor="#585858"></u-icon>
				</view>
            ...
            background: #fff5ee;
          ">
					<text class="u-font-24 color-text-orange u-m-r-8 u-m-l-16">我的收益</text>
		...
		<view class="u-flex-row u-m-28" style="justify-content: space-between">
			<view v-if="des1T" class="u-flex-col u-m-r-16 card-bg" style="padding-top: 16rpx; padding-bottom: 16rpx">
          ...
          des1T
			...
			<view v-if="des2T" class="u-flex-col card-bg" style="padding-top: 16rpx; padding-bottom: 16rpx">
				<text class="u-font-bold color-text-black u-font-32 u-line-h-48">{{
          des2
        }}</text>
				<text class="color-text-less-grey u-font-24 u-line-h-40">{{
		...
		<view class="card-bg u-flex-row u-m-28 u-col-center u-row-between" v-show="showSource">
			<view class="u-flex-row u-col-center">
          ...
          sourceName
			...
			<view>
				<text class="u-font-22 color-text-primary u-m-r-16 btn-bg u-line-h-32" @click="clipLink()">复制链接</text>
				<text class="u-font-22 color-text-primary btn-bg u-line-h-32" @click="openLink()">直接打开</text>
			</view>
		</view>
		<view class="title-sitcky u-m-t-48 u-p-b-28">
			<text class="u-font-bold u-font-32 u-text-main u-m-l-28"> 推广计划</text>
		</view>
		<view :style="{
        paddingLeft: '28rpx',
        paddingRight: '28rpx',
        paddingBottom: `${btnHeight}`,
			...
			<view class="u-flex-col u-col-center" style="width: 100%; margin-top: 136rpx" v-show="planList.length == 0">
				<u-image :src="image.no_data_list" width="360rpx" height="360rpx" shape="square"></u-image>
				<text class="u-font-24 color-text-less-grey"
					style="position: relative; bottom: 45px">暂无计划，请点击下方新建按钮</text>
			</view>
			<template v-if="planList.length">
				<view v-for="(item, index) in planList" :key="index" class="card-bg u-m-b-16 u-relative">
					<view class="u-flex-row u-col-center u-row-between" style="height: 40rpx">
						<view class="u-flex-row u-col-center" style="background: #f6f6f6; border-radius: 8rpx">
							<u-icon size="28rpx" :name="item.platform_icon" class="u-m-r-8 u-m-l-8"></u-icon>
							<text class="u-line-h-40 u-m-r-8 u-font-20 u-text-main">{{
                item.type_name
              }}</text>
						</view>
						<text class="u-m-l-16 u-font-28 u-text-main u-flex-1 u-line-1"
							style="width: 440rpx">{{ item.name }}</text>
						<radio v-show="isDeletMode" :value="item.value" :checked="item.isSelected"
							activeBackgroundColor="#408CFF" borderColor="#C6C6C6" @click="doCheck(item)"
							style="transform: scale(0.6);position: absolute; right: 15rpx; top: 20rpx; z-index: 2" />
					</view>
					<view v-if="item.type == 3 && item.self_operated==1" class="u-flex-row u-col-center u-m-t-16">
						<u-icon :bold="true" :name="`${static_path}icon_key.png`" width="32rpx" height="32rpx" label="口令" labelSize="24rpx" labelColor="#3C3C3C" space="8rpx"></u-icon>
						<u-line
						  ...
						  style="opacity: 0.6;"
						></u-line>
						<view class="u-font-24 color-text-less-black">{{item.search_code}}</view>
					</view>
					<view class="u-flex-row u-m-t-16 u-col-center" style="
              border-radius: 12rpx;
              border: 1rpx solid #ff7736;
              background: #fff5ee;
            ">
						<view class="u-flex-col u-col-center"
							...
							<text
								class="u-font-bold u-font-28 u-line-h-44 color-text-orange">{{ getResult1(item) }}</text>
							<text class="u-font-22 u-line-h-40 color-text-orange">{{
                `${item.charge_data ? "支付订单" : "CPM收益"}`
              }}</text>
						</view>
						<u-line color="#C6C6C6" direction="col" length="16rpx"></u-line>
						<view v-if="item.charge_data" class="u-flex-col u-col-center"
							style="padding-top: 16rpx; padding-bottom: 16rpx; flex: 1">
							<text class="u-font-bold u-font-28 u-line-h-44 color-text-orange">{{
                  unitMoney(item.charge_data.charge_price, false, false)
                }}</text>
							<text class="u-font-22 u-line-h-40 color-text-orange">充值金额</text>
						</view>
						<u-line v-if="item.charge_data && item.cpm_data" color="#C6C6C6" direction="col"
							length="16rpx"></u-line>
						<view v-if="item.charge_data&& item.cpm_data" class="u-flex-col u-col-center"
							style="padding-top: 16rpx; padding-bottom: 16rpx; flex: 1">
							<text class="u-font-bold u-font-28 u-line-h-44 color-text-orange">{{
                  unitMoney(item.charge_data.total_price, false, false)
                }}</text>
							<text class="u-font-22 u-line-h-40 color-text-orange">CPS预估收益</text>
						</view>
						<u-line v-if="item.charge_data" color="#C6C6C6" direction="col" length="16rpx"></u-line>
						<view v-if="item.charge_data" class="u-flex-col u-col-center"
							style="padding-top: 16rpx; padding-bottom: 16rpx; flex: 1">
							<text
								class="u-font-bold u-font-28 u-line-h-44 color-text-orange">{{ getResult2(item) }}</text>
							<text class="u-font-22 u-line-h-40 color-text-orange">{{
              ...
              item.create_time
						...
						<text class="u-line-h-40 u-font-24 u-p-l-24 u-p-r-24 u-p-t-4 u-p-b-4 color-text-white"
							@click="doPromotion(item)"
							:style="{opacity: item.type == 3 && item.self_operated==1&&(item.verify_status!=3&&item.verify_status!=4)?0.5:1,borderRadius:'200rpx',background:item.type == 3 && item.self_operated==1&&item.verify_status==4?'#F6F6F6':'#408cff'}">{{ item.button_text }}</text>
						<u-icon v-if="item.type==3&&item.self_operated==1"
							:name="`${static_path}${getDisplayLabel(item)}.png`" width="80rpx" height="80rpx"
							class="u-absolute" style="top: 0rpx; right: 0rpx;"></u-icon>
					</view>
				</view>
			</template>
			<u-loadmore v-if="!listLoading && planList.length > 0" :status="status" :loading-text="loadingText"
				:loadmore-text="loadmoreText" :nomore-text="nomoreText"></u-loadmore>
		</view>
		<BottomBtn :data="buttonList" :buttonIndex="buttonIndex" :layout="188" @delete="deletePlan"
			@height="getBtnHeight" @creatPlan="creatPlan" @manage="onManage" @cancel="onCancel" />
		<PromotionPopup ref="popupContainer" :showWordCode="showWordCode" :typeList="typeList"
			:accountList="accountList" :submitLoading="submitLoading" :autoClose="false" @completed="onPopupCompleted">
		</PromotionPopup>
		<PromotionTaskPropup ref="taskPropup" :qrcode="qrcode" :scheme="scheme" :platformType="platformType">
		</PromotionTaskPropup>
		<u-modal :show="showDeleteNotcie" @confirm="confirmDelete" @cancel="cancelDelete" ref="uModal" title="温馨提示"
			width="404rpx" content="删除后无法恢复，确认删除吗?" confirmText="确认删除" confirmColor="#408CFF" cancelColor="#6A6A6A"
	...
	export default {
		...
		methods: {
			...
			getDisplayLabel(item) {
				...
				return statusMap[item.verify_status] || "icon_audit_status_ing";
			},
			getResult1(item) {
...
generatePlan: throttle(async function fuc(
  ...
  try {
    ...
    if (type == 2) {
      params.platform_account_primary_id = platform_account_primary_id;
    }
    ...
    this.$refs.popupContainer.closePromotionPopup();
  }
}, 500),
			//删除推广计划
						...
						if (item.type === 3) {
							if(item.self_operated==1){
								switch(item.verify_status){
									...
									break
									case 4:
									this.failReason = item.verify_suggest;
									this.showReason = true;
									break
									default:
									this.toastMsg("口令审核中，待通过后可进行复制", "error");
									break
								}
