    ...
    <view class="top-area">
      <view class="top-area--navbar u-flex-row u-col-center u-p-x-28 u-p-y-16">
        <u--image
          ...
          @click="goBack"
        ></u--image>
        <text class="color-text-white u-font-32 u-line-h-48 u-m-l-16"
          >版权书库</text
        >
      </view>
      <u--image
        :src="`${static_path}book_bg.png`"
        width="750rpx"
        height="360rpx"
      ></u--image>
      <view class="top-area--tab u-p-t-28 u-p-b-32 u-p-x-28">
      ...
      <view v-if="listData.length" class="book-list u-p-x-28">
        ...
        >
          ...
          <view class="item-info u-flex-col u-row-center">
            <text
              class="u-font-28 u-font-bold u-line-h-44 color-text-black u-m-b-16"
              >{{ item.name }}</text
            >
...
import { getLibrarySelect } from "@/api/public.js";
...
export default {
  ...
  methods: {
    ...
    getTypeList() {
      ...
      getLibrarySelect({ type: 1 })
        .then((res) => {
          if (res.code == 0) {
            this.tabList = [{ name: "全部", value: 0 }, ...res.data];
          }
        })
        .catch((error) => {
