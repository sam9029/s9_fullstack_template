 ...
 */
import props from "@/uni_modules/uview-ui/components/u-picker/props.js";
export default {
  name: "u-picker",
  ...
  methods: {
    getSearch(val) {
      ...
      if (this.remote) {
        ...
        return this.$emit('remote', val);
      };
      if (!this.remote) {
        if (val) {
          let cloum = this.columns[0].filter(
            (i) =>
              String(i.label).indexOf(val) != -1 ||
              String(i.value).indexOf(val) != -1
          );
          this.setColumns([cloum]);
        } else {
