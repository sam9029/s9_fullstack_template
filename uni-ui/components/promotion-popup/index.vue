<template>
	<view>
		<u-modal :show="showAddAccountNotcie" @confirm="confirmNotice"  @cancel="cancelNotice" ref="uModal" title="温馨提示" width="404rpx"
			content="您在当前选择平台下还未添加账号，请前往 个人中心 -> 我的账户 页面新增账号后再试!" confirmText="立即前往"  confirmColor="#408CFF" cancelColor="#6A6A6A"
		...
		<u-popup :show="showPromotionPopup" round="24rpx">
			<view class="popup-content">
				<view class="title-box u-flex-row">
					...
					<u-icon :name="`${static_path}close_circle_grey.png`" width="24" height="24"
						@click="closePromotionPopup"></u-icon>
				</view>

				<view class="name-box u-flex-row">
					<text class="name-box-title u-font-28 ">计划名称</text>
					<u-input class="name-box-input u-font-24 color-text-black u-p-l-36" v-model="planName"
						placeholder="请输入推广计划名称" placeholderStyle="color: #989898;font-size: 24rpx;"></u-input>
				</view>

				<view class="name-box u-flex-row">
					<text class="name-box-title u-font-28 ">推广方式</text>
					<view class="name-box-input u-flex-row u-flex-between" @click="openTypePopup">
						<text class="u-font-24 u-flex-1"
							:style="{'color':selectedType=='请选择推广方式'?'#989898':'#1A1A1A'}">{{selectedType}}</text>
				...
				<view class="name-box u-flex-row" v-show="showAccount">
					...
					<view class="name-box-input u-flex-row u-flex-between" @click="openAccountPopup">
						<text class="u-font-24 u-flex-1"
							:style="{'color':selectedAccount=='请选择平台账号'?'#989898':'#1A1A1A'}">{{selectedAccount}}</text>
						<u-icon size="16rpx" name="arrow-down"></u-icon>
					</view>
				</view>
				<view class="name-box u-flex-row" v-show="showWord">
					<text class="name-box-title u-font-28 ">文字口令</text>
					<u-input class="name-box-input u-font-24 color-text-black u-p-l-36" v-model="wordsCode"
						placeholder="请输入4-8位文字口令" placeholderStyle="color: #989898;font-size: 24rpx;"></u-input>
				</view>
				<view class="u-m-t-8" style="margin-left: 140rpx;" v-show="showWord">
					<text class="u-font-22"
						style="color: #FA4753;">注意：此项非必填项，若不填写，系统将自动生成口令；文字口令需审核通过才生效，系统口令则立即生效。</text>
				</view>
				<button class="submit-btn u-font-32 u-font-weight color-text-white u-bg-primary"
					@click="handlePopupComplate">提交</button>
			</view>
		</u-popup>
		...
		<u-popup :show="showTypePopup" round="24rpx">
			<view class="popup-content">
				<view class="title-box u-flex-row">
					<text class="color-text-black u-font-32 u-font-weight">请选择推广方式</text>
					<u-icon :name="`${static_path}close_circle_grey.png`" width="24"
					height="24" @click="closeTypePopup()"></u-icon>
				</view>

				<picker-view class="picker-view-box" :style="{height : pickerHeight + 'rpx'}"
					indicator-style="height: 96rpx;" :value="pickervalue" @change="onPickerChange">
					<picker-view-column>
						<view class="picker-view-item u-font-32 color-text-black u-font-weight"
							v-for="(item,index) in typeList" :key="item['id'] || index">
							{{item['name']}}
						</view>
					</picker-view-column>
				...
				<view class="type-submit-box u-flex-row">
					<view class="type-submit-box-btn u-font-28 u-font-weight color-text-less-grey"
						@click="closeTypePopup">取消</view>
					<view class="type-submit-box-space"></view>
					<view class="type-submit-box-btn u-font-28 u-font-weight color-text-primary"
						@click="selectTypeComplate">确定</view>
				</view>
			</view>
		</u-popup>
		<!-- 弹框 C -->
		<u-popup :show="showAccountPopup" round="24rpx">
			<view class="popup-content">
				<view class="title-box u-flex-row">
					<text class="color-text-black u-font-32 u-font-weight">请选择平台账号</text>
					<u-icon :name="`${static_path}close_circle_grey.png`" width="24"
						height="24" @click="closeAccountPopup()"></u-icon>
				</view>

				<picker-view class="picker-view-box" :style="{height : pickerHeight + 'rpx'}"
					indicator-style="height: 96rpx;" :value="pickerAccountvalue" @change="onAccountPickerChange">
					<picker-view-column>
						<view class="picker-view-item u-font-32 color-text-black u-font-weight"
							v-for="(item,index) in accountList" :key="item['id'] || index">
							{{item['name']}}
						</view>
					</picker-view-column>
				...
				<view class="type-submit-box u-flex-row">
					<view class="type-submit-box-btn u-font-28 u-font-weight color-text-less-grey"
						@click="closeAccountPopup">取消</view>
					<view class="type-submit-box-space"></view>
					<view class="type-submit-box-btn u-font-28 u-font-weight color-text-primary"
						@click="selectAccountComplate">确定</view>
				</view>
			</view>
		</u-popup>
					...
					if (this.selectedTypeId == 2) {
						if (this.selectedAccontId == -1) {
							...
							const data = {
								...
								accountId: this.selectedAccontId
							}
							this.$emit("completed", data);
						}
						...
						const data = {
							name: this.planName,
							id: this.selectedTypeId,
							keyword: this.wordsCode
						}
						this.$emit("completed", data);
					} else {
						const data = {
							name: this.planName,
							id: this.typeList[this.selectedIndex]['id']
						}
						console.log(data)
						this.$emit("completed", data);
					}
