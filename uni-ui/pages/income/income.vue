  ...
  <view class="income-page">
    <view class="income-page--container u-p-x-28">
      <view class="account-info base-radius u-m-b-28 u-p-32 color-text-white">
        <view class="u-flex-row u-row-between u-col-center u-m-b-32">
          ...
          <text
            class="u-font-24 u-line-h-40 color-text-disabled-primary"
            @click="jumpPage('withdrawRecord')"
            >提现记录 ></text
          >
        </view>
        <view class="u-flex-row u-row-between u-col-center u-p-b-32">
          <view class="u-flex-col">
            <text class="u-font-24 u-line-h-40 color-text-disabled-primary"
              >可提现(元)</text
            >
            <text class="u-font-bold u-font-60 u-line-h-76">{{
              unitMoney(topCard.limit_amount, false, true)
            }}</text>
          </view>
          <view
            class="u-bg-f u-p-x-16 u-p-y-4 u-flex-row u-col-center u-row-center"
            style="border-radius: 100px; height: 48rpx;width: 98rpx;"
            @click="jumpWithdraw"
          >
        ...
        <view
          class="widthAll"
          style="border-bottom: 2rpx solid #fff; opacity: 10%; height: 2rpx"
        ></view>
        <view class="info--bottom--price u-m-t-32">
          <view class="u-flex-col color-text-white" @click="jumpPage('withdrawRecord')">
            <text class="u-font-24 u-line-h-40 color-text-disabled-primary"
              >已提现 ></text
            >
            <text class="u-font-40 u-line-h-56 u-font-bold">{{
              unitMoney(topCard.amount_cash_out, false, true)
            }}</text>
          </view>
      ...
      <view class="task-income u-bg-f base-radius u-m-b-28 u-p-32 u-flex-col">
        <text class="u-font-24 u-line-h-40 color-text-less-black"
          >做任务变现收益</text
        >
        <view
          class="u-flex-row u-row-between u-m-t-48"
          @click="jumpPage('novel')"
        >
          <view class="u-flex-row u-col-top">
            <u-icon
              :name="`${static_path}income_novel_icon.png`"
              size="48rpx"
            ></u-icon>
            <view class="u-flex-col u-m-l-16">
              ...
              >
                <text>{{
                  novelObj.total_amount ? "累计收益" : "暂无收益"
                }}</text>
                <text v-if="novelObj.total_amount" class="u-m-l-16">{{
          ...
          <view class="u-flex-row u-col-center">
            <text class="color-text-less-grey u-font-24 u-line-h-40 u-m-r-8"
              >关键词推文变现</text
            >
        ...
        <view
          class="u-flex-row u-row-between u-m-t-48"
          @click="jumpPage('demand')"
        >
          <view class="u-flex-row u-col-top">
            <u-icon
              :name="`${static_path}income_demand_icon.png`"
              size="48rpx"
            ></u-icon>
            <view class="u-flex-col u-m-l-16">
              <text class="u-font-28 u-line-h-48 u-font-bold">看点推广</text>
              <view
                v-if="demandObj.be_credited_amount || demandObj.total_amount"
                class="u-flex-row u-col-center color-text-less-grey u-font-24 u-line-h-40"
              >
                <view class="u-flex-row u-col-center">
                  <text>待入帐: </text>
                  <text class="color-text-orange u-m-l-16">{{
              ...
              <text v-else class="color-text-less-grey u-font-24 u-line-h-40"
                >暂无收益</text
              >
            </view>
          </view>
      ...
      <view
        class="activity-income u-bg-f base-radius u-m-b-28 u-p-32 u-flex-col"
      >
        <text class="u-font-24 u-line-h-40 color-text-less-black"
          >参与活动奖励收益</text
        >
        <view
          class="u-flex-row u-row-between u-m-t-48"
          @click="jumpPage('activity')"
        >
          <view class="u-flex-row u-col-top">
            <u-icon
              :name="`${static_path}income_activity_icon.png`"
              size="48rpx"
            ></u-icon>
            <view class="u-flex-col u-m-l-16">
              ...
              >
                <text>{{
                  activityObj.total_amount ? "累计收益" : "暂无收益"
                }}</text>
                <text v-if="activityObj.total_amount" class="u-m-l-16">{{
      ...
      <view class="create-income u-bg-f base-radius u-flex-col u-p-32">
        ...
        >
          <view class="u-flex-row u-col-top">
            <u-icon
              :name="`${static_path}income_create_icon.png`"
              size="48rpx"
            ></u-icon>
            <view class="u-flex-col u-m-l-16">
              <text class="u-font-28 u-line-h-48 u-font-bold">承制收益</text>
              <view
                v-if="createObj.be_credited_amount || createObj.total_amount"
                class="u-flex-row u-col-center color-text-less-grey u-font-24 u-line-h-40"
              >
                <view class="u-flex-row u-col-center">
                  <text>待入帐: </text>
    ...
    <u-popup :show="showAddWithdraw" round="16rpx" mode="center">
      <view class="u-p-48" style="width: 520rpx">
        <view class="u-font-32 u-font-weight u-text-main">请添加提现账户</view>
        <view class="u-font-28 color-text-less-black u-m-t-32"
          >为避免打款错误，需先实名绑定收款信息后方可提现</view
        >
...
import { getWalletTotal } from "@/api/pages/income.js";
import { getBankList } from "@/api/public.js";
  ...
  onShow() {
    if(!this.has_login) {
      return uni.navigateTo({
        url: '/pages/login/login?from=INCOME'
      })
    }
    this.getData();
  },
