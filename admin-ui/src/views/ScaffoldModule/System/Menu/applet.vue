...
<script>
  ...
  import Form from './appletForm.vue';
  import { applet_columns } from './data';
  export default {
    ...
    methods: {
      ...
      loadChild(tree, treeNode, resolve) {
        ...
        listMenu({ pid: tree.id, router_type: 2 }).then((res) => {
          resolve(res.data);
        });
      },
      getTableData() {
      ...
      /** 查询菜单下拉树结构 */
      getTreeselect() {
        listMenu({ type: 'tree', menu_type: 'F', router_type: 2 }).then((res) => {
          this.menuOptions = [];
