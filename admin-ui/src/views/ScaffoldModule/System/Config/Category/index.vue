    ...
    >
      <template #header_left>
        <el-button
          type="primary"
          @click="addNew"
          :disabled="!$checkPermi(['sys:config:category:add'])"
          >新增品类</el-button
        >
      </template>
    </BaseTable>

    <BaseDialog title="作品品类" :visible.sync="visible" width="600px" @close="close">
...
<script>
  ...
  import { list, create, remove, update, updateStatus } from '@/api/system/category.js';
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(row) {
        this.$confirm(`确认删除品类 ${row.name} ?`, '提示', {
          type: 'warning',
          callback: (action) => {
            if (action == 'confirm') {
              remove({ ids: [row.id] })
      ...
      submit() {
        ...
        if (!name) return this.$notify.warning('请输入品类名称！');
