    ...
    <template v-if="mode == 'transfer'">
      ...
      <div class="transfer-center">
        <template v-if="button_text">
          ...
          <p class="transfer-center-item">
            <el-button @click="removeToSource" :disabled="to_disabled" icon="el-icon-arrow-left">{{
              toButton || '移除'
            }}</el-button>
          </p>
        </template>
      ...
      <div class="transfer-right">
        <h3 class="transfer-title">
          ...
          <template v-if="transferRightHeaderTool">
            <el-radio-group v-model="radio" @change="childRadio">
              <el-radio label="allowed">指定</el-radio>
              <el-radio label="forbiden">排除</el-radio>
            </el-radio-group>
          </template>
        </h3>
        <!-- 内容区 -->
      ...
      <div class="transfer-right">
        ...
        >
          <h3 class="transfer-title">
            ...
            <img
              ...
              v-else
              src="https://koc-img.lizhibj.cn/manage/logo.png"
              alt
              @click="moveUp('down')"
            />
          </h3>
        ...
        >
          <h3 class="transfer-title">
            ...
            <img class="move_up_img" v-else src="https://koc-img.lizhibj.cn/manage/logo.png" alt @click="moveUp('up')" />
          </h3>
          <!-- 内容区 -->
          <div class="transfer-main" v-if="move_up">
            <!-- <slot name='to'></slot> -->
...
<script>
  import { arrayToTree } from '@/utils/common/treeTransfer/array.js';
  export default {
    data() {
      return {
    ...
    // },
    // mounted () {
    //   console.log('mounted', this.defaultCheckedKeys)
    // },
    methods: {
      // 添加按钮
      addToAims() {
      ...
      addressListTransfer(type) {
        ...
        switch (type) {
          ...
          case 2:
            arrayDeWeighting = arrayCheckedNodes.filter((item) => {
              if (!this.secret_receiver.some((ite) => ite[this.node_key] == item[this.node_key])) {
                return item;
              }
            });
      ...
      clearList(type, id) {
        switch (type) {
          case 0:
            ...
            break;
          case 1:
            this.Cc = id == 'all' ? [] : this.Cc.filter((item) => item[this.node_key] != id);
            break;
          case 2:
      ...
      childRadio() {
        this.$emit('childRadio', this.radio);
      },
    },
    computed: {
      // 左侧数据
      self_from_data() {
        ...
        if (!this.arrayToTree) {
          ...
          this.flag = from_array.length ? true : false;
          setTimeout(() => {
            this.addToAims();
          }, 0);
          return from_array;
        } else {
          const res = arrayToTree(from_array, {
            id: this.node_key,
            pid: this.pid,
            children: this.defaultProps.children,
          });
          this.flag = res.length ? true : false;
          return res;
        }
      },
      // 右侧数据
