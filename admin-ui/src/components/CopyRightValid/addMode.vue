...
<script>
  import { postAddCopyrightValid } from '@/api/copyrightCenter/drama/validLog.js';
  ...
  export default {
    ...
    methods: {
      ...
      handleOriginChange(val) {
        let unResetField = ['content_type', 'name', 'author', 'uid', 'file', 'end_date'];
      ...
      submitDialog(data, str = '') {
        ...
        postAddCopyrightValid({ ...removeObjectEmpty(params) })
          .then((res) => {
            if (res.code == 0) {
